...
comparison_bar_rmse <- function(L0L2=rmse_res_Wang,G_SMuRFS=rmse_res_l0)
{
  ...
  for(j in 1:length(rmse_res_Wang[[1]]))#ROI数量
  {
    ...
    y2=c()
    if(j %% 3 == 1) {
      print(paste("D:\\test\\AD\\2-ADNI\\supplementary\\Data_S1_", floor(j / 3) + 1, ".png", sep=""))
      png(file=paste("D:\\test\\AD\\2-ADNI\\supplementary\\Data_S1_", floor(j / 3) + 1, ".png", sep=""), 
...
n <- nrow(xx)
cols <- rev(gray(0:(n + 1)/(n + 1)))[1:n]
barplot(xx, col = cols, beside = TRUE,
...
######
save(rmse_res_Wang,rmse_res_l0,file='bgsmtr_example_data_results.Rdata')
load('2-ADNI/bgsmtr_example_data_results.Rdata')
