...
test_MTR <- function(X, folders, Y, Group) {
  ...
  for (i in 1:folder_no) {
    ...
    fit <- Wang_CV_tuning_values_FUN(X[, train_idx],
      Y[, train_idx],
      group = Group
    )
...
fit <- Wang_CV_tuning_values_FUN(
  X = bgsmtr_example_data$SNP_data,
  Y = bgsmtr_example_data$BrainMeasures,
  group = bgsmtr_example_data$SNP_groups
)
W_estimate <- fit$W_Wang_from_tuning_CV
y_prediction <- predictMTR(W_estimate, bgsmtr_example_data$SNP_data[, 1:20]) # All SNP of top 20 samples
RMSE_MTR(bgsmtr_example_data$BrainMeasures[, 1:20], y_prediction) # Test accuracy of 15 ROI on top 20 samples
